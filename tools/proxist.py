...
from db_work.models import soc_db, Proxy
...
class ProxyGrabber():
    ...
    def check(self):
        ...
        for proxy in Proxy.select().where(Proxy.level == 1):
            proxy_body = proxy.body.split(':')
            speed = self.test_connection(proxy_body)
            proxy.level = speed
            proxy.save()
        print('Done checking')
        Proxy.delete().where(Proxy.level == 0)

    def commit(self):
        soc_db.connect()
        soc_db.create_table(Proxy, safe=True)
